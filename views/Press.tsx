...
export const Press: React.FC = () => {
  ...
  const { content, site, localizedCopy } = useContent();
    ...
    <div className="grain relative pt-40 pb-20 bg-artbar-bg min-h-screen">
      <div className="max-w-[1400px] mx-auto px-6 md:px-10">
        <div className="text-center mb-16">
           <span className="text-artbar-taupe font-heading font-bold tracking-widest text-sm uppercase mb-4 block"><JpText>{site.pressPage.badge}</JpText></span>
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-heading font-heavy text-artbar-navy mb-6"><JpText>{site.pressPage.title}</JpText></h1>
          <p className="text-lg md:text-xl text-artbar-gray"><JpText>{site.pressPage.subtitle}</JpText></p>
        </div>
        ...
        <div ref={carouselReveal.ref} className={`reveal relative mb-32 group ${carouselReveal.isVisible ? 'visible' : ''}`}>
          <div className="hidden md:block">
            <button 
              ...
              aria-label={stripJpSentinel(localizedCopy.ui.press.scrollLeft)}
            >
              <ChevronLeft size={24} />
            </button>
            ...
            <button 
              ...
              aria-label={stripJpSentinel(localizedCopy.ui.press.scrollRight)}
            >
            ...
            {content.media.map((item, idx) => (
              ...
              >
                ...
                <div className="absolute inset-0 p-6 flex flex-col justify-end text-white">
                  ...
                  <div>
                    <h3 className="font-heading font-bold text-2xl mb-2 leading-tight drop-shadow-sm"><JpText>{item.outlet}</JpText></h3>
        ...
        <div ref={timelineReveal.ref} className={`reveal bg-artbar-navy text-white rounded-[3rem] p-8 md:p-24 shadow-2xl relative overflow-hidden ${timelineReveal.isVisible ? 'visible' : ''}`}>
          ...
          />
          <h2 className="text-3xl md:text-4xl font-heading font-bold mb-16 relative z-10"><JpText>{site.pressPage.popupsTitle}</JpText></h2>
          <div className="border-l-2 border-white/20 pl-8 md:pl-10 space-y-16 relative z-10">
            {site.pressPage.popups.map((event, i) => (
              <div key={i} className="relative">
                ...
                <h3 className="text-xl md:text-2xl font-heading font-bold mb-2"><JpText>{event.title}</JpText></h3>
                <div className="flex items-center gap-4 text-artbar-light-taupe">
                    ...
                    <span className="text-sm md:text-base"><JpText>{event.loc}</JpText></span>
                </div>
              </div>
            ))}
          </div>
