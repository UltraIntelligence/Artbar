...
- The hero viewport uses a static image from `images.hero.home`; `images.hero.video` and `images.hero.videoMobile` feed the concept/lifestyle block, not the full-screen hero background. Home applies a slow CSS keyframe drift to that hero background (`hero-bg-drift` in `views/Home.tsx`); it is disabled when the user prefers reduced motion.
