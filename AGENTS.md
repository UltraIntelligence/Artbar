...
- Full-bleed hero images on theme detail, team building, blog posts, and private parties reveal after load with brand placeholders. Home hero is not hidden behind `opacity-0` until load (iOS can skip those events); `bg-artbar-navy` sits under the image/video instead. Home hero adds `onLoad`, redundant video events, and a ~2.8s reveal fallback so iOS does not leave the hero invisible if callbacks never fire.
