...
- Full-bleed hero images and video on home, theme detail, team building, blog posts, and private parties reveal after load (`onLoadingComplete` / `onLoadedData`) with brand-colored placeholders so backgrounds do not flash empty black or navy before assets arrive. Home hero adds `onLoad`, redundant video events, and a ~2.8s reveal fallback so iOS does not leave the hero invisible if callbacks never fire.
