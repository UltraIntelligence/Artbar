...
- Partner logo grids (`PartnerLogo` on home and team building): on mobile use shorter logo rows, narrower cells, tighter grid gaps, and smaller rendered marks so the strip does not feel oversized.
...
- The concept block social avatar strip uses curated Unsplash face-crop URLs (`CONCEPT_SOCIAL_AVATAR_URLS` in `views/Home.tsx`); include `images.unsplash.com` in `next.config.ts` `remotePatterns` when serving them through `next/image`.
