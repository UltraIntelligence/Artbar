...
- For headline social proof (guest counts, ratings), prefer exact figures the user provides over rounded shorthand when credibility matters. Do not put `whitespace-nowrap` on the hero guest-count line when the copy is long (especially Japanese); use a full-width proof row with `min-w-0` on the flex children so text can wrap, and for Japanese add `break-keep` with `overflow-wrap: anywhere` as needed. Long lines should not sit cramped beside star ratings; stack rows and/or use slightly smaller type and tracking when needed.
