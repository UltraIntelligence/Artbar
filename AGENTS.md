...
- Full-bleed hero images on theme detail, team building, blog posts, and private parties reveal after load with brand placeholders. Home hero is not hidden behind `opacity-0` until load (iOS can skip those events); `bg-artbar-navy` sits under the image/video. Home uses a two-layer wash (navy from below, taupe from above) for contrast over the hero media—do not reintroduce JS-gated hero opacity without a hard fallback.
