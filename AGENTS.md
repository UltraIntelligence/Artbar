...
- **Heading font:** Poppins (400–700) — class `font-heading`, global `h1–h3`
- **Body font:** Noto Sans JP (400/500/700) — `font-sans`, with system UI and Hiragino/Meiryo fallbacks in `globals.css`
...
- Partner logo grids (`PartnerLogo` on home and team building): below `sm`, keep a short row height (`h-10` prominent) and scale most marks down with `max-sm:scale-*` so wordmarks do not dominate; lift GE and Apple (and tune Toyota) on small screens with `max-sm:`/`sm:` pairs so desktop from `sm` keeps the prior per-brand scales. Home keeps logos in a white card with slightly wider mobile grid gaps (`gap-x-5 gap-y-9`); team building can omit the card but should match similar logo size, weight, and spacing to the home strip.
- When an icon sits next to a multi-line marketing label, use layout that keeps icon and text visually one unit (often column on small screens); pick an icon that matches the meaning, or omit it if it does not clarify the message.
...
- Full-bleed hero images on theme detail, team building, blog posts, and private parties reveal after load with brand placeholders. Home hero is not hidden behind `opacity-0` until load (iOS can skip those events); `bg-artbar-navy` sits under the image/video. Home uses a two-layer wash (navy from below, taupe from above) and layered `text-shadow` on hero h1/subtitle for contrast—do not reintroduce JS-gated hero opacity without a hard fallback. When nudging readability without replacing photography, favor small shifts toward warmer taupe / softer navy in the wash rather than a cold blue-heavy gradient that fights white headline type.
- On touch viewports, show explanatory copy that desktop reveals on hover (for example theme card subtitles under titles). Home Popular Themes cards need enough card height and bottom padding that three-line descriptions (`line-clamp-3`) stay above the rounded corners; adjust the title-area gradient stop (e.g. from ~38% mobile / ~32% md+) rather than clipping copy.
...
- For Japanese-only typography fixes, keep English copy and layout unchanged; scope rules with `html:lang(ja)` (set `document.documentElement.lang` to `ja` when the app language is `jp`), use `word-break: keep-all` and `overflow-wrap: anywhere` on headings and body text in `globals.css`, and use `<wbr>` in Japanese hero strings for intentional mobile line breaks where needed. Do not shrink hero heading sizes to fix wrapping; adjust breaks or copy before reducing display type.

## Learned Workspace Facts

- Tailwind v4 maps `font-heading` and `font-sans` to `--font-heading` and `--font-sans`; `--font-family-heading` / `--font-family-sans` style names do not generate those utilities. `next/font` exposes `--font-poppins` and `--font-noto-sans-jp` on `<html>`; `@theme` references them in `--font-heading` / `--font-sans`. `ThemeInjector` sets `--font-heading` / `--font-body` to the admin-chosen family names (quoted). Default heading/body are bundled via `next/font`; `ThemeInjector` skips duplicate Google Fonts CSS for Poppins and Noto Sans JP.
...
- The hero viewport uses a still image by default (`HERO_HOME_FALLBACK` in `constants.ts`); `images.hero.home` can point to GIF or MP4 instead. `images.hero.video` and `images.hero.videoMobile` feed the concept/lifestyle block, not the full-screen hero background. MP4 heroes use a full-bleed looping video; GIF heroes skip the CSS drift (`hero-bg-motion`); still images keep the drift (`hero-bg-drift` in `globals.css`), disabled when the user prefers reduced motion. The concept block may use looped in-page video with a control that opens a longer clip in a new tab (e.g. YouTube) when embedding would cause letterboxing or layout issues.
- `lib/gemini-image-config.ts` holds the default Gemini image model (`gemini-3.1-flash-image-preview`) and related generation options; override the model with `GEMINI_IMAGE_MODEL` when needed. For theme detail **example painting** slots (`theme-*-example-[1–4]`), generation passes **`imageConfig`** with **`imageSize` `1K`** and **`aspectRatio` `1:1`**; set **`GEMINI_THEME_EXAMPLE_IMAGE_SIZE`** to `2K` or `4K` if you need sharper exports. Prompt art direction: finished canvases in bright minimal rooms (white walls, simple context), not easels or busy wood studios; master-themed slots should evoke specific named works in spirit (palette, composition), not generic period pastiche.
- Root `app/layout.tsx` uses `suppressHydrationWarning` on `<html>` and `<body>` so browser extensions that inject attributes before hydrate (for example `cz-shortcut-*`) do not surface React hydration mismatch warnings in development.
