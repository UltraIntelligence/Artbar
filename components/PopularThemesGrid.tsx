...
export function PopularThemesGrid({ items }: { items: ThemeListItem[] }) {
  return (
      ...
      {items.map((themeItem) => {
        ...
        return (
          <Link
            key={slug}
            href={`/themes/${slug}`}
            className="group relative block h-[300px] md:h-[500px] rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden cursor-pointer shadow-sm hover:shadow-2xl transition-all duration-500"
          >
            <img
              src={themeItem.image}
              alt={themeItem.title}
            ...
            <div className="absolute inset-x-0 bottom-0 p-4 md:p-10 w-full">
              <div className="transition-transform duration-300 ease-out group-hover:-translate-y-1">
                ...
                <p className="text-white/85 text-[10px] sm:text-xs md:text-sm font-light leading-snug md:leading-relaxed line-clamp-2 min-h-[2.5rem] md:min-h-[3rem]">
                  {themeItem.desc}
                </p>
              </div>
            </div>
